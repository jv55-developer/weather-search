...
function App() {
  ...
  const [ radioButton, setRadioButton ] = useState(radio)
  ...
  const handleLatLon = (e) => {
    ...
    axios.get(`https://api.api-ninjas.com/v1/geocoding?city=${location}`, config)
    .then((res) => {
      setLocationOptions(res.data.slice(0, 4))
      locationOptions.map(object => {
        return object.image = radioButton
      })
  ...
  const handleRadioButton = (lat, lon, city, province) => {
    setRadioButton(radio_alt)

    axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${openWeatherKey}`)
    .then(res => {
        ...
        <div className="row">
          <div className="col">
            ...
            <form onSubmit={e => handleLatLon(e)} className="mb-5">
              ...
              <div className="mb-3">
                {!checking && <button className="btn btn-purple rounded-pill">Check Weather</button>}
                {checking && <button disabled type="submit" className="btn btn-purple rounded-pill"><ThreeCircles
                  height="20"
                  width="20"
                  color="#ffffff"
            ...
            {locationOptions.map(item => {
              return <div className="location-card px-3 py-2 mb-4" key={item.latitude}>
                <div className="first-section">
                  <img src={buildings} alt='animated location graphic' className="img-responsive img-city my-auto" />
                  <div className="mx-4">
                    <p>
                      {item.name}<br />
                      {item.state}<br />
                ...
                <img onClick={() => handleRadioButton(item.latitude, item.longitude, item.name, item.state)} src={item.image} alt='animated location graphic' className="img-responsive img-radio my-auto" />
              </div>
            })}
          </div>
          <div className="col location-container">
            {Object.keys(weather).length > 0 && (
              <>
                <div className={`${weather.weather[0].main.toLowerCase()} weather-image`}></div>
                <div className="p-4">
                  <h3>{city}</h3>
                  <h5>{province}</h5>
                  <hr />
